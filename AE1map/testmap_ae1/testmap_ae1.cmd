...
set AE1title=Ancient Empires - Map tester batch
title %AE1title% [%1]

echo.
echo.
echo batch file from byblo, for AE1 map editor. 200x
...
if not exist temp\%ae1temp%.jar goto error_temp_ae1temp_jar

::extract the game archive content to the temp\ folder (only the files which do not exist yet)
bin\unzip.exe -n temp\%ae1temp%.jar -d temp\
...
bin\zip.exe -u -j temp\%ae1temp%.jar temp\*.class temp\gameicon.png temp\lang.dat temp\1.pak
echo ------------------------------------------------
...
del temp\%ae1temp%.exe
::if cannot be removed, maybe it is still running, then give error message
if exist temp\%ae1temp%.exe goto ae1temp_exe_cannot_del

::compile .jar to .exe (this will permit to keep the game's setting into the ae1temp-properties\ folder created by executable)
bin\midp2exe.exe -jar temp\%ae1temp%.jar -o temp\%ae1temp%.exe
echo ------------------------------------------------
...
if not exist temp\%ae1temp%.exe goto ae1temp_exe_cannot_create
temp\%ae1temp%.exe
goto end_no_pause
...
::run game
java.exe -jar bin\player.jar temp\%ae1temp%.jar
...
:error_temp_ae1temp_jar
echo.
echo Error: file temp\%ae1temp%.jar could not be found!
echo.
echo You will need to test your map a JAR game file,
echo to rename and to place it to the temp\%ae1temp%.jar
goto end
